...
test("deployment query maps real site plans and operations without fixtures", () => {
  ...
  const plan = { planId, nodeId: "node-production-01", deploymentEnvironment: "staging", domains: ["stackpilot.example.com"], repositoryUrl: "https://github.com/example/stackpilot.git", repositoryRef: "main", certificateEnvironment: "production", environmentVariableNames: [], operator: "管理员", status: "ready", digest: "a".repeat(64), version: 1, preview: { runtime: "node22", healthCheckPath: "/healthz", changes: ["repository"] }, operationId, createdAt: now, updatedAt: now, expiresAt: "2026-07-15T00:30:00.000Z" };
...
test("deployment query ignores mismatched and non-deployment operations", () => {
  ...
  const planId = "22222222-2222-4222-8222-222222222222";
  const operationId = "11111111-1111-4111-8111-111111111111";
  const plan = { planId, nodeId: "node-production-01", deploymentEnvironment: "production", domains: ["stackpilot.example.com"], repositoryUrl: "https://github.com/example/stackpilot.git", repositoryRef: "feature/not-an-environment", certificateEnvironment: "production", environmentVariableNames: [], operator: null, status: "ready", digest: "a".repeat(64), version: 1, preview: null, operationId, createdAt: now, updatedAt: now, expiresAt: "2026-07-15T00:30:00.000Z" };
