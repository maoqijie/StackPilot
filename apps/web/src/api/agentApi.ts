import type { AgentNodeRecord, CreateEnrollmentRequest, RemoteTaskRecord } from "@stackpilot/contracts";
import { requestJson } from "./client";

export type EnrollmentCredential = { enrollmentId: string; token: string; expiresAt: string; purpose: "agent-enrollment" };
export const listAgentNodes = (signal?: AbortSignal) => requestJson<{ nodes: AgentNodeRecord[] }>("/nodes", { signal });
...
export const listRemoteTasks = (signal?: AbortSignal) => requestJson<{ tasks: RemoteTaskRecord[] }>("/remote-tasks", { signal });
