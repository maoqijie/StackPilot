...
function BackupSummary({ rows }: { rows: DatabaseInstance[] }) {
  const summary = [
    ...
    { label: "待确认 / 运行中", count: rows.filter((row) => row.backupStatus === "等待确认" || row.backupStatus === "运行中").length, tone: "orange", Icon: CircleAlert },
...
function HealthMini({ instance }: { instance: DatabaseInstance | null }) {
  ...
  return (
    <div className="health-mini">
      ...
      <p><span>采集时间</span><b>不可用</b><em>本地会话数据</em></p>
    </div>
  );
}
...
function SlowInstanceList({ rows, onOpen }: { rows: DatabaseInstance[]; onOpen: (id: string) => void }) {
  const slowInstances = [...rows]
    .filter((row) => row.slowQueries > 0)
    .sort((left, right) => right.slowQueries - left.slowQueries)
    .slice(0, 5);
    ...
    <div className="database-watch-list">
      {slowInstances.map((instance) => (
        <button key={instance.id} type="button" onClick={() => onOpen(instance.id)}>
          ...
          <strong>{instance.slowQueries} 条</strong>
        </button>
      ))}
    </div>
  );
...
function AccessSummary({ rows }: { rows: DatabaseInstance[] }) {
  const accessTypes: DatabaseInstance["access"][] = ["读写", "只读", "仅备份"];
  return (
    <div className="database-summary-list">
