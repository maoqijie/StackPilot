...
function useLiveTerminal(permissions: Permission[]) {
  ...
  const [hostCollectedAt, setHostCollectedAt] = useState<string | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null); const inFlight = useRef<Promise<void> | null>(null); const active = useRef<AbortController | null>(null); const hasData = useRef(false); const taskVersion = useRef(0); const localTaskVersions = useRef(new Map<string, number>());
  const load = useCallback((externalSignal?: AbortSignal, silent = false) => {
    ...
    const requestTaskVersion = taskVersion.current; const requests = [
    ...
    const request = Promise.allSettled(requests).then(([nodeResult, hostResult, taskResult]) => {
      ...
      if (taskResult.status === "fulfilled") setTasks((current) => { const merged = new Map(taskResult.value.tasks.map((task) => [task.taskId, task])); for (const task of taskResult.value.tasks) localTaskVersions.current.delete(task.taskId); for (const task of current) { const version = localTaskVersions.current.get(task.taskId); if (version && version > requestTaskVersion) merged.set(task.taskId, task); else if (version) localTaskVersions.current.delete(task.taskId); } return [...merged.values()]; });
  ...
  const create = useCallback(async (nodeId: string, input: CreateRemoteTaskRequest, proof: string) => { const task = await createTerminalTask(nodeId, input, proof); taskVersion.current += 1; localTaskVersions.current.set(task.taskId, taskVersion.current); setTasks((current) => [task, ...current.filter((item) => item.taskId !== task.taskId)]); return task; }, []);
