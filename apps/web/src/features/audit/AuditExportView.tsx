...
function AuditExportConfirm({ pending, resource, notify, onClose }: { pending: PendingAction; resource: AuditExportsResource; notify: Notify; onClose: () => void }) {
  ...
  const reconcileRecord = async (record: AuditExportRecord) => {
    resource.replaceData({
      collectedAt: resource.data?.collectedAt ?? record.completedAt ?? record.createdAt,
      exports: [record, ...(resource.data?.exports ?? []).filter((row) => row.id !== record.id)],
    });
    await resource.refresh();
  };
  const submit = async () => { if (!password || busy || (pending.kind === "create" && !name.trim())) return; setBusy(true); setError(""); try { const proof = (await reauthenticate(password)).proof; if (pending.kind === "create") { const record = await createAuditExport({ name: name.trim(), format: exportFormat }, proof); await reconcileRecord(record); notify(record.status === "ready" ? "真实审计快照已生成" : "审计快照生成失败，可在列表重试", record.status === "ready" ? "success" : "warning"); } else if (pending.kind === "retry") { const record = await retryAuditExport(pending.record.id, proof); await reconcileRecord(record); notify(record.status === "ready" ? "审计快照已重新生成" : "审计快照重试失败", record.status === "ready" ? "success" : "warning"); } else { await downloadAuditExport(pending.record, proof); notify(`${pending.record.name} 已开始下载`, "success"); } onClose(); } catch (caught) { setError(caught instanceof Error ? caught.message : "审计导出操作失败"); } finally { setBusy(false); } };
