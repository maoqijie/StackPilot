...
import { purgeFileTrash, purgeTrashEntry, restoreTrashEntry, type TrashEntry } from "../../api/filesApi";
...
type Confirmation = { type: "file"; row: TrashEntry; returnToDetail: boolean } | { type: "all"; count: number };
...
function FileTrashPage({ page, notify, canManage = true }: { page: PageKey; notify: Notify; canManage?: boolean }) {
  ...
  const ownerOptions = ["全部", ...new Set(trashRows.map((row) => row.owner))];
  const filteredRows = trashRows.filter((row) => {
    const query = search.trim().toLowerCase();
    return (!query || `${row.name} ${row.originalPath} ${row.reason} ${row.owner}`.toLowerCase().includes(query)) && (ownerFilter === "全部" || row.owner === ownerFilter);
  });
  ...
  const restoreFile = async (row: TrashEntry) => {
    ...
    setSubmitting(true);
    try {
      const result = await restoreTrashEntry(row.id);
      setPayload(result.trash);
      if (selectedId === row.id) setSelectedId("");
      notify(result.message);
    } catch (reason) { notify(reason instanceof Error ? reason.message : "恢复失败", "danger"); }
    finally { setSubmitting(false); }
  };
  const requestPurge = (row: TrashEntry, returnToDetail = false) => {
    if (returnToDetail) setSelectedId("");
    setPassword(""); setMutationError(null); setConfirmation({ type: "file", row, returnToDetail });
  };
  const closeConfirmation = () => {
    ...
    setPassword(""); setMutationError(null); setConfirmation(null);
  };
  const confirmPurge = async () => {
    ...
    try {
      ...
      setPayload(result.trash); setSelectedId(""); setConfirmation(null); setPassword(""); notify(result.message, "danger");
    } catch (reason) { setMutationError(reason instanceof Error ? reason.message : "永久删除失败"); }
    finally { setSubmitting(false); }
  };
  const formatDate = (value: string) => new Date(value).toLocaleString("zh-CN", { hour12: false });
  const formatSize = (row: TrashEntry) => row.kind === "directory" ? "文件夹" : row.sizeBytes === null ? "暂不可用" : formatUploadSize(row.sizeBytes);
  ...
  return <>
    <div className="trash-page-layer" inert={Boolean(selected || confirmation)} aria-hidden={selected || confirmation ? "true" : undefined}>
      <ModulePageShell title={resolvePageMeta(page).title} subtitle="独立回收站视图，支持按所有者筛选、查看删除原因、恢复文件和永久删除。" page={page}
        ...
        actions={<><button className="ghost" type="button" onClick={() => { restorePanelRef.current?.scrollIntoView({ block: "start" }); restorePanelRef.current?.querySelector<HTMLElement>("h2")?.focus({ preventScroll: true }); }}><RefreshCw size={15} /> 查看恢复记录</button>{canManage && <button className="trash-destructive" type="button" disabled={!trashRows.length || submitting} onClick={() => { setPassword(""); setMutationError(null); setConfirmation({ type: "all", count: trashRows.length }); }}><Trash2 size={15} /> 清空回收站</button>}</>}
        ...
        side={selected && <DetailDrawer title="删除详情" subtitle={selected.name} onClose={() => setSelectedId("")} className="trash-detail-drawer" modal actions={canManage ? <><button className="trash-destructive" type="button" disabled={submitting} aria-label={`永久删除 ${selected.name}`} onClick={() => requestPurge(selected, true)}>永久删除</button><button className="primary" type="button" disabled={submitting} aria-label={`恢复 ${selected.name}`} onClick={() => void restoreFile(selected)}>恢复</button></> : undefined}><dl className="trash-detail-list"><div><dt>原路径</dt><dd>{selected.originalPath}</dd></div><div><dt>大小</dt><dd>{formatSize(selected)}</dd></div><div><dt>删除时间</dt><dd>{formatDate(selected.deletedAt)}</dd></div><div><dt>剩余保留</dt><dd>{expiresIn(selected.expiresAt)}</dd></div><div><dt>所有者</dt><dd>{selected.owner}</dd></div><div><dt>删除原因</dt><dd>{selected.reason}</dd></div></dl></DetailDrawer>}
        ...
        <div className="file-trash-workspace">
          ...
          {payload && <DataTable columns={[
            ...
            { key: "size", label: "大小", width: "84px", render: formatSize }, { key: "deleted", label: "删除时间", render: (row) => formatDate(row.deletedAt) },
