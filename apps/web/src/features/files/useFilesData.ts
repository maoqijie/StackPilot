...
export function useFilesData(path:string,onResolvedPath?:(path:string)=>void){const[data,setData]=useState<FileListPayload|null>(null),[errorState,setErrorState]=useState<{path:string;message:string}|null>(null);const requestRef=useRef(0),controllerRef=useRef<AbortController|null>(null),resolvedPathRef=useRef(onResolvedPath);
  useEffect(()=>{resolvedPathRef.current=onResolvedPath;},[onResolvedPath]);
  const run=useCallback(async(controller:AbortController,request:number)=>{try{const next=await fetchFiles(path,controller.signal);if(request===requestRef.current){setData(next);setErrorState(null);if(!path)resolvedPathRef.current?.(next.path);}}catch(reason){if(controller.signal.aborted)return;if(request===requestRef.current)setErrorState({path,message:reason instanceof Error?reason.message:"目录读取失败"});}},[path]);
