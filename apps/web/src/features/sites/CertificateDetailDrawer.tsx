import { CalendarClock, CheckCircle2, RefreshCw, Shield, ShieldAlert } from "lucide-react";
...
function CertificateDetailDrawer({ site, canRenew, onClose, onRenew }: { site: SiteRuntimeView; canRenew: boolean; onClose: () => void; onRenew: () => void }) {
  const StatusIcon = site.certificate.status === "valid" ? CheckCircle2 : ShieldAlert;
  return <DetailDrawer
  ...
  >
    <div className={`cert-detail-status ${certificateRiskTone(site)}`}><StatusIcon size={22} /><div><strong>{certificateRiskLabel(site)} · {certificateRemainingLabel(site)}</strong><span>{freshnessLabel(site)} · 采集于 {formatBackendDateTime(site.collectedAt)}</span></div></div>
