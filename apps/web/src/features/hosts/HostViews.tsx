...
function HostMetrics({ mode, rows, filteredRows }: { mode: HostPageMode; rows: HostView[]; filteredRows: HostView[] }) {
  ...
  if (mode === "alerts") return <><MetricTile icon={Shield} label="待处理" value={`${rows.filter(hostNeedsAttention).length}`} tone="orange" /><MetricTile icon={WifiOff} label="离线" value={`${rows.filter((row) => row.status === "离线").length}`} tone="orange" /><MetricTile icon={Clock3} label="备份异常" value={`${rows.filter(hostHasStaleBackup).length}`} tone="orange" /></>;
...
function HostStatus({ row }: { row: HostView }) {
  const Icon = row.status === "健康" ? CheckCircle2 : row.status === "警告" ? TriangleAlert : row.status === "离线" ? WifiOff : Wrench;
...
function HostResourceSummary({ row }: { row: HostView }) {
  return <span className="host-resource-summary" aria-label={`${metricAria("CPU", row.cpu)}，${metricAria("内存", row.memory)}，${metricAria("磁盘", row.disk)}`}>
    {[["CPU", row.cpu], ["内存", row.memory], ["磁盘", row.disk]].map(([label, value]) => <span key={label as string}><em>{label}</em><MetricBar label={label as string} value={value as number | null} /></span>)}
  </span>;
}
...
function HostServices({ row }: { row: HostView }) { return <span className="host-service-pills">{row.services.length ? row.services.slice(0, 3).map((service) => <em key={service.id}>{service.name}</em>) : <em>未配置</em>}</span>; }

function DiskMetric({ row }: { row: HostView }) {
  const tooltipId = useId();
  if (row.disk === null) return <span className="host-metric-unavailable">等待采集</span>;
  return <span className="host-disk-metric"><button type="button" className="host-disk-trigger" aria-label={`磁盘使用率 ${metricText(row.disk)}`} aria-describedby={row.diskVolumes.length ? tooltipId : undefined}><MetricBar label="磁盘" value={row.disk} /></button>{row.diskVolumes.length > 0 && <span className="host-disk-tooltip" id={tooltipId} role="tooltip"><strong>全部磁盘卷</strong>{row.diskVolumes.map((volume) => <span key={`${volume.label}-${volume.mount}`}><b>{volume.label} ({volume.mount})</b><em>{volume.percent}% · 已用 {formatBytes(volume.usedBytes)} / {formatBytes(volume.totalBytes)}</em></span>)}</span>}</span>;
}
...
function HostDetailContent({ host }: { host: HostView }) {
  ...
  return <div className="host-detail-content">
    ...
    <div className="detail-kv"><p><span>节点 ID</span><b>{host.id}</b></p><p><span>连接状态</span><b>{host.status}</b></p><p><span>版本</span><b>{host.version}</b></p><p><span>运行时间</span><b>{host.uptime}</b></p><p><span>网络延迟</span><b>{host.latency}</b></p><p><span>更新</span><b>{host.update}</b></p><p><span>备份</span><b>{host.backup}</b></p><p><span>负责人</span><b>{host.owner}</b></p><p><span>最近在线</span><b>{formatTimestamp(host.lastSeenAt)}</b></p></div>
