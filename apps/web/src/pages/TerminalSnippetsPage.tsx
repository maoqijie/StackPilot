...
import { executeTerminalSnippet } from "../api/terminalApi";
...
import { useTerminalSnippets } from "../features/terminal/useTerminalSnippets";
...
function TerminalSnippetsPage({ notify }: { notify: Notify }) {
  const { snippets, nodes, tasks, collectedAt, loading, error, retry, toggleFavorite, acceptExecution } = useTerminalSnippets();
  ...
  const run = (snippet: TerminalSnippetRecord) => {
    ...
    setSelectedId(null); setPendingId(snippet.id); setPassword("");
  };
  const confirmExecution = async () => {
    ...
    try {
      ...
      const idempotencyKey = createLocalId(`terminal-${pending.id}`);
      const result = await executeTerminalSnippet(pending.id, { nodeId: effectiveNode.nodeId, snippetVersion: pending.version, idempotencyKey }, proof.proof);
      acceptExecution(result.snippet, result.task); setExecutionTaskId(result.task.taskId); setPendingId(null); setPassword("");
      notify(`${pending.title} 已提交给 ${effectiveNode.nodeName}`, "success");
    } catch (reason) {
      notify(reason instanceof Error ? reason.message : "受控命令提交失败", "danger");
    } finally { setExecuting(false); }
  };
  const closeExecutionDialog = useCallback(() => {
    if (executing) return;
    setPendingId(null); setPassword("");
  }, [executing]);

  return <>
    <ModulePageShell
      title={resolvePageMeta("terminal-snippets").title}
    ...
    >
      ...
      {error && <div className="overview-error-state terminal-snippets-error"><Shield size={18} /><span>{error}</span><button type="button" onClick={() => void retry()}>重试</button></div>}
      <div className="terminal-snippets-live-layout">
        <section className="terminal-snippet-library" aria-label="受控命令片段">
          {filtered.map((snippet) => {
            const canRun = Boolean(effectiveNode && snippet.executable && availableFor(effectiveNode, snippet));
            return <article key={snippet.id} className={snippet.favorite ? "favorite" : ""}>
              <header><button className="terminal-snippet-title" type="button" onClick={() => setSelectedId(snippet.id)}><strong>{snippet.title}</strong><Eye size={16} /></button><span className={`pill terminal-snippet-risk ${snippet.risk === "danger" ? "red" : snippet.risk === "change" ? "orange" : "green"}`}>{snippet.risk === "read" ? <Shield size={13} /> : <AlertTriangle size={13} />}{riskLabel[snippet.risk]}</span></header>
              <button className="terminal-snippet-command" type="button" aria-label={`查看 ${snippet.title} 详情`} onClick={() => setSelectedId(snippet.id)}><code>{snippet.command}</code></button>
              <p>{snippet.description}</p>
              <footer><span><b>{snippet.category}</b><time>{snippet.lastUsedAt ? formatBackendDateTime(snippet.lastUsedAt) : "尚未执行"}</time></span><div>
                <button className={`icon-action${snippet.favorite ? " active" : ""}`} type="button" aria-label={`${snippet.favorite ? "取消收藏" : "收藏"} ${snippet.title}`} title={snippet.favorite ? "取消收藏" : "收藏"} onClick={() => void toggleFavorite(snippet).then((updated) => notify(`${updated.title} 已${updated.favorite ? "收藏" : "取消收藏"}`, "info")).catch((reason: unknown) => notify(reason instanceof Error ? reason.message : "收藏状态保存失败", "danger"))}><Star size={16} fill={snippet.favorite ? "currentColor" : "none"} /></button>
                <button className="icon-action" type="button" aria-label={`复制 ${snippet.title}`} title="复制命令" onClick={() => copy(snippet.command)}><Copy size={16} /></button>
                <button className="primary small" type="button" aria-label={`执行 ${snippet.title}`} disabled={!canRun} onClick={() => run(snippet)}><Play size={15} />{snippet.executable ? "执行" : "未接入"}</button>
              </div></footer>
            </article>;
          })}
          {!loading && filtered.length === 0 && <p className="module-empty-card">{error ? "真实片段加载失败，未显示示例数据" : "没有匹配的受控命令"}</p>}
        </section>
        <aside className="terminal-live-task-panel" aria-live="polite">
          <header><Clock3 size={18} /><span><strong>最近任务</strong><small>{latestTask ? formatBackendDateTime(latestTask.updatedAt) : "等待首次执行"}</small></span></header>
          {latestTask ? <div className={`terminal-live-task-result ${taskTone(latestTask.status)}`}>{taskIcon(latestTask.status)}<span><strong>{taskStatusLabel[latestTask.status]}</strong><small>{effectiveNode?.nodeName ?? latestTask.targetNodeId}</small></span><code>{latestTask.taskId}</code><p>{latestTask.result?.message ?? latestTask.errorCode ?? "任务已保存，等待 Agent 返回结果"}</p>{latestTask.result?.data && <pre>{JSON.stringify(latestTask.result.data, null, 2)}</pre>}</div> : <p className="module-empty-card">选择在线 Agent 后执行受控片段，真实任务状态将在此处自动更新。</p>}
        </aside>
      </div>
    </ModulePageShell>
