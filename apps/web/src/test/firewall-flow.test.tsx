...
describe("firewall rule flow", () => {
  ...
  it("opens deny details as a body-level modal drawer", async () => {
    ...
    expect(within(drawer).getByText("来源不在运维白名单")).toBeInTheDocument();
  });

  it("requires confirmation before allowing a denied source", async () => {
    const user = userEvent.setup();
    const notify = vi.fn();
    render(<FirewallPage page="firewall-deny" notify={notify} />);

    const table = screen.getByRole("table");
    await user.click(within(table).getByRole("button", { name: "放行拦截来源 198.51.100.24" }));
    ...
    await waitFor(() => expect(screen.queryByRole("alertdialog", { name: "确认放行来源" })).not.toBeInTheDocument());
    expect(within(table).getByRole("button", { name: "放行拦截来源 198.51.100.24" })).toBeDisabled();
    expect(notify).toHaveBeenCalledWith("198.51.100.24 已放行", "info");
  });
});
