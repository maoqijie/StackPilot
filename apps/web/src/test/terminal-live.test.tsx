...
describe("terminal real backend integration", () => {
  ...
  it("renders real Agent identity, address and command output", async () => { render(<TerminalPage page="terminal" notify={vi.fn()} />); expect(await screen.findByText("real-agent-01")).toBeInTheDocument(); expect(screen.getByText("198.18.0.10")).toBeInTheDocument(); expect(screen.getByText("12:30:03 up 1 day, 1:01, 1 user, load average: 0.10, 0.20, 0.30")).toBeInTheDocument(); expect(screen.getByText("exit code: 0")).toBeInTheDocument(); expect(screen.queryByText("panel-se-01")).not.toBeInTheDocument(); });
  it("reauthenticates before submitting a structured task", async () => { render(<TerminalPage page="terminal" notify={vi.fn()} />); const input = await screen.findByLabelText("命令输入"); fireEvent.change(input, { target: { value: "systemctl status nginx --no-pager" } }); fireEvent.click(screen.getByRole("button", { name: "运行" })); const dialog = screen.getByRole("alertdialog", { name: "确认执行受控命令" }); fireEvent.change(within(dialog).getByLabelText("当前密码"), { target: { value: "current-password" } }); fireEvent.click(within(dialog).getByRole("button", { name: "确认执行" })); await vi.waitFor(() => expect(createTerminalTask).toHaveBeenCalledWith(node.nodeId, expect.objectContaining({ type: "terminal.command.execute", parameters: { command: "service-status", serviceName: "nginx" } }), "proof-value-with-more-than-thirty-two-characters")); });
  ...
  it("keeps a queued terminal command in progress until an exit code arrives", async () => { vi.mocked(fetchTerminalTasks).mockResolvedValue({ tasks: [{ ...task, status: "queued", result: null }] }); render(<TerminalPage page="terminal-history" notify={vi.fn()} />); expect(await screen.findByText("执行中")).toBeInTheDocument(); expect(screen.queryByText("失败")).not.toBeInTheDocument(); });
  ...
  it("does not consume reauthentication when the selected Agent lacks a command capability", async () => {
    ...
    vi.mocked(fetchTerminalNodes).mockResolvedValue({ nodes: [limited] });
    const notify = vi.fn();
  ...
  it("removes task history that the authoritative backend no longer returns", async () => {
    ...
    render(<TerminalPage page="terminal-history" notify={vi.fn()} />);
    expect(await screen.findByText("12:30:03 up 1 day, 1:01, 1 user, load average: 0.10, 0.20, 0.30")).toBeInTheDocument();
    fireEvent(document, new Event("visibilitychange"));
    await vi.waitFor(() => expect(fetchTerminalTasks).toHaveBeenCalledTimes(2));
    await vi.waitFor(() => expect(screen.queryByText("12:30:03 up 1 day, 1:01, 1 user, load average: 0.10, 0.20, 0.30")).not.toBeInTheDocument());
  ...
  it("reuses the task idempotency key when a submission response is lost", async () => {
    ...
    fireEvent.change(within(dialog).getByLabelText("当前密码"), { target: { value: "current-password" } });
    fireEvent.click(within(dialog).getByRole("button", { name: "确认执行" }));
    expect(await within(dialog).findByText("响应连接中断")).toBeInTheDocument();
    fireEvent.click(within(dialog).getByRole("button", { name: "确认执行" }));
    await vi.waitFor(() => expect(createTerminalTask).toHaveBeenCalledTimes(2));
    const first = vi.mocked(createTerminalTask).mock.calls[0][1];
    const second = vi.mocked(createTerminalTask).mock.calls[1][1];
