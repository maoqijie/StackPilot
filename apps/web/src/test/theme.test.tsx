...
describe("permission-aware audit navigation", () => {
  ...
  it("shows real audit navigation with audit:read", () => { expect(navItemsForPermissions(["audit:read"]).some((item) => item.key === "audit")).toBe(true); expect(topbarSearchResults("审计", ["audit:read"]).some((item) => item.page === "audit-export")).toBe(true); });
});
