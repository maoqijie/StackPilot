...
export class TaskExecutor {
  ...
  private running = new Map<string, { controller: AbortController; cancellable: boolean }>();
  ...
  async execute(raw: unknown, onRunning?: (update: RemoteTaskStatusUpdate) => Promise<void>): Promise<RemoteTaskStatusUpdate> {
    ...
    const controller = new AbortController(); this.running.set(task.taskId, { controller, cancellable: definition.cancellable }); const timer = setTimeout(() => controller.abort(), definition.timeoutMs);
