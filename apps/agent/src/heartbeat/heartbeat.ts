...
export function createHeartbeat(config: AgentConfig, nodeId: string, capabilities: AgentHeartbeat["capabilities"], telemetry?: AgentTelemetrySnapshot, telemetryCollectionFailed = false, siteSnapshot?: AgentSiteSnapshot, databaseSnapshot?: AgentDatabaseSnapshot, physicalHostId?: PhysicalHostId, systemdSnapshot?: AgentSystemdSnapshot): AgentHeartbeat {
  let uptimeSeconds = 0;
  ...
  return {
    ...
    ...(telemetry ? { telemetry } : {}), ...(siteSnapshot ? { siteSnapshot } : {}), ...(databaseSnapshot ? { databaseSnapshot } : {}), ...(systemdSnapshot ? { systemdSnapshot } : {}),
  };
}
