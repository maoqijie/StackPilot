...
import { AGENT_PROTOCOL_VERSION, type AgentDatabaseSnapshot, type AgentHeartbeat, type AgentTelemetrySnapshot } from "@stackpilot/contracts";
import type { AgentConfig } from "../config/environment.js";
import { telemetryIsDegraded } from "../telemetry/collector.js";
export function createHeartbeat(config: AgentConfig, nodeId: string, capabilities: AgentHeartbeat["capabilities"], telemetry?: AgentTelemetrySnapshot, telemetryCollectionFailed = false, databaseSnapshot?: AgentDatabaseSnapshot): AgentHeartbeat {
  let uptimeSeconds = 0;
  ...
  return {
    ...
    ...(telemetry ? { telemetry } : {}), ...(databaseSnapshot ? { databaseSnapshot } : {}),
  };
}
