...
import { assertPortAvailable, SiteStateStore, waitForManagedPortOwner } from "./siteState.js";
    ...
    try {
      if (site.manifest.runtime !== "static" && desiredState === "running") {
        if (site.desiredState !== "running") {
          await run("/usr/bin/systemctl", ["stop", unitName(siteId)], 30_000); await assertPortAvailable(site.port);
        }
        await run("/usr/bin/systemctl", ["start", unitName(siteId)], 30_000);
        const state = await run("/usr/bin/systemctl", ["show", "--property=ActiveState", "--property=SubState", "--property=MainPID", "--property=ControlGroup", unitName(siteId)], 20_000);
