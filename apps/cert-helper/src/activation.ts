...
const portFor = (id: string) => 20_000 + Number.parseInt(id.slice(-4), 16) % 20_000;
...
export async function activatePlan(plan: PreparedPlan, config: HelperConfig, dependencies: Dependencies = {}) {
  ...
  const site: ManagedSite = { siteId: id, planId: plan.planId, domains: plan.domains, manifest: plan.manifest, releaseId: plan.releaseId, port: plan.manifest.runtime === "static" ? null : portFor(id), desiredState: "running", protected: plan.domains.some((domain) => config.protectedDomains.has(domain)), version: (existing?.version ?? 0) + 1, certificateName, runtimePath: plan.runtimePath, createdAt: existing?.createdAt ?? now, updatedAt: now };
  try {
