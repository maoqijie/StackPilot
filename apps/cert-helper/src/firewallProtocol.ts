...
type FirewallRequest = FirewallHelperRequest;
...
function firewallRequest(raw: string): FirewallRequest {
  return parseFirewallRequest(raw);
}
...
export async function handleFirewallRequest(raw: string, dependencies: FirewallDependencies = {}): Promise<HelperResponse> {
  ...
  try { request = firewallRequest(raw); }
  ...
  const started = performance.now(); const config = dependencies.config ?? loadConfig();
  try {
    const data = request.operation === "firewall-list" ? await (dependencies.list ?? listFirewallRules)()
      : request.operation === "firewall-create" ? await (dependencies.create ?? createFirewallRule)(request, config.stateRoot)
        : await (dependencies.delete ?? deleteFirewallRule)(request, config.stateRoot);
