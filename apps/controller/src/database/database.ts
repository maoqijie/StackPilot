...
export function openDatabase(path: string): Database.Database {
  ...
  try {
    migrateDatabase(database, [
      ...
      { version: 4, name: "file-trash", sql: readFileSync(resolve(migrationDirectory,"004_file_trash.sql"), "utf8") },
      { version: 5, name: "file-trash-storage", sql: readFileSync(resolve(migrationDirectory,"005_file_trash_storage.sql"), "utf8") },
    ]);
  } catch (error) {
    database.close();
    throw error;
