...
async function routeSchedules(context: RequestContext) {
  ...
  if (parts.length === 3 && method === "POST") { const payload = parseSchema(CreateScheduleJobRequestSchema, context.body, "请求体"); authorizeMutation(); const result = await services.schedules.create(payload); sendJson(response, 201, { ...result, message: `${result.job.name} 已写入当前用户 crontab`, tone: "success" }, ScheduleMutationResponseSchema); return; }
  if (parts.length === 4 && method === "PATCH") { const id = idAt(context, 3); const run = RunScheduleJobRequestSchema.safeParse(context.body); const update = run.success ? null : parseSchema(UpdateScheduleJobRequestSchema, context.body, "请求体"); authorizeMutation(); const result = run.success ? await services.schedules.run(id) : await services.schedules.update(id, update!); sendJson(response, 200, { ...result, message: `${result.job.name} ${run.success ? "已立即执行" : "已保存到当前用户 crontab"}`, tone: result.job.result === "失败" ? "warning" : "success" }, ScheduleMutationResponseSchema); return; }
