...
export async function routeFileManagerRequest(context:RequestContext){const{request,response,parts,services,url}=context,method=request.method??"GET";
  if(parts.length===2&&method==="GET"){context.identity?.require(context.principal,"files:read");if(url.searchParams.size>1||(url.searchParams.size===1&&!url.searchParams.has("path")))throw badRequest("查询参数无效");const path=url.searchParams.has("path")?parseSchema(FilePathSchema,url.searchParams.get("path"),"文件路径"):services.fileManager.defaultRoot();sendJson(response,200,await services.fileManager.list(path),FileListPayloadSchema);return;}
  ...
  if(parts.length===2&&method==="DELETE"){const input=parseSchema(DeleteFileRequestSchema,context.body,"删除文件");const name=await services.fileManager.moveToTrash(input.path,context.principal?.user.username??context.principal?.userId??"unknown");sendJson(response,200,{message:`${name} 已移入回收站`,entry:null},FileMutationResponseSchema);return;}
