...
export async function routeFileManagerRequest(context:RequestContext){const{request,response,parts,services,url}=context,method=request.method??"GET";
  ...
  if(parts.length===2&&method==="DELETE"){const input=parseSchema(DeleteFileRequestSchema,context.body,"删除文件");if(!services.fileTrash)throw new ApiError(503,"NOT_READY","文件回收站服务未配置");const entry=await services.fileTrash.move(input.path,context.principal?.user.displayName??context.principal?.userId??"unknown");sendJson(response,200,{message:`${entry.name} 已移入回收站`,entry:null},FileMutationResponseSchema);return;}
