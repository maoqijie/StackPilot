...
export async function routeFileRequest(context:RequestContext){const{request,response,parts,services,url}=context,method=request.method??"GET";
  if(parts.length===2&&method==="GET"){context.identity?.require(context.principal,"files:read");const path=parseSchema(FilePathSchema,url.searchParams.get("path"),"文件路径");sendJson(response,200,await services.files.list(path),FileListPayloadSchema);return;}
  ...
  if(parts[2]==="upload"&&parts.length===3&&method==="POST"){const path=parseSchema(FilePathSchema,request.headers["x-file-path"],"目标目录"),name=parseSchema(FileNameSchema,request.headers["x-file-name"],"文件名");const entry=await services.files.upload(path,name,context.rawBody);sendJson(response,201,{message:`${entry.name} 已上传`,entry},FileMutationResponseSchema);return;}
