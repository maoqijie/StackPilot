...
export async function routeAuditExportRequest(context: RequestContext): Promise<void> {
  ...
  context.identity?.require(principal, "audit:read");
  ...
  if (context.parts.length === 2 && method === "POST") {
    ...
    const record = await service.create(input, { userId: principal.userId, displayName: principal.user.displayName }, context.requestId);
  ...
  if (context.parts.length === 4 && context.parts[3] === "retry" && method === "POST") {
    ...
    consumeReauthentication(context);
    const record = await service.retry(exportId, principal.userId, isAdministrator, { userId: principal.userId, displayName: principal.user.displayName }, context.requestId);
    sendJson(context.response, 201, { export: record }, AuditExportCreateResponseSchema);
    return;
  }
  if (context.parts.length === 4 && context.parts[3] === "download" && method === "POST") {
    const exportId = parseSchema(PathIdSchema, context.parts[2] ?? "", "导出 ID");
    consumeReauthentication(context);
    const { record, handle } = await service.download(exportId, principal.userId, isAdministrator, context.requestId);
    ...
    context.response.setHeader("Content-Length", record.sizeBytes);
    await pipeline(handle.createReadStream({ autoClose: true }), context.response);
    return;
  }
