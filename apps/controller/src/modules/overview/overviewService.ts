...
type ResourceAggregate = {
  ...
  loadAverages: number[]; liveNodes: number;
};
...
export class OverviewService {
  ...
  async getOverview(access: OverviewAccess = fullAccess, options: { bypassCache?: boolean } = {}): Promise<OverviewSummaryPayload> {
    ...
    const onlineTelemetry = remoteNodes.filter((node) => this.freshness(node) === "current" && node.telemetry).map((node) => node.telemetry!);
    const aggregate = this.aggregateResources(local.snapshot, onlineTelemetry);
    ...
    return OverviewSummaryPayloadSchema.parse({
      ...
      metrics: [
        { label: "CPU 使用率", value: aggregate.cpu === null ? "暂不可用" : String(aggregate.cpu), suffix: aggregate.cpu === null ? "" : "%", delta: aggregate.cpu === null ? "等待采集" : `${aggregate.cpuSamples.length} 个核心样本`, icon: "server", tone: aggregate.cpu === null ? "gray" : aggregate.cpu >= 85 ? "red" : aggregate.cpu >= 70 ? "orange" : "blue", line: aggregate.cpuSamples },
        { label: "内存使用率", value: aggregate.memory === null ? "暂不可用" : String(aggregate.memory), suffix: aggregate.memory === null ? "" : "%", delta: aggregate.memory === null ? "等待采集" : `${formatGb(Math.max(0, aggregate.memoryTotal - aggregate.memoryUsed))} 可用`, icon: "database", tone: aggregate.memory === null ? "gray" : aggregate.memory >= 88 ? "red" : aggregate.memory >= 76 ? "orange" : "blue", line: aggregate.memory === null ? [] : spark(aggregate.memory) },
        { label: "磁盘使用率", value: aggregate.disk === null ? "暂不可用" : String(aggregate.disk), suffix: aggregate.disk === null ? "" : "%", delta: aggregate.disk === null ? "等待采集" : `${aggregate.diskVolumes} 个盘 · ${formatGb(Math.max(0, aggregate.diskTotal - aggregate.diskUsed))} 可用`, icon: "globe", tone: aggregate.disk === null ? "gray" : aggregate.disk >= 90 ? "red" : aggregate.disk >= 80 ? "orange" : "blue", line: aggregate.disk === null ? [] : spark(aggregate.disk), details: this.diskDetails(local.snapshot, onlineTelemetry) },
  ...
  private aggregateResources(snapshot: PlatformSnapshot, telemetry: AgentTelemetrySnapshot[]): ResourceAggregate {
    ...
    const loads = [snapshot.loadAverages, ...telemetry.flatMap((item) => item.loadAverage ? [item.loadAverage] : [])];
    const loadAverages = [0, 1, 2].map((index) => loads.reduce((sum, item) => sum + (item[index] ?? 0), 0) / Math.max(loads.length, 1));
    return { cpuSamples, cpu, memoryTotal, memoryUsed, memory: percent(memoryUsed, memoryTotal), diskTotal, diskUsed, disk: percent(diskUsed, diskTotal), diskVolumes: volumes.length, loadAverages, liveNodes: 1 + telemetry.length };
  }
