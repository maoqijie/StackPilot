...
export class OverviewService {
  ...
  async getOverview(): Promise<OverviewSummaryPayload> {
    ...
    const resources: Record<string, OverviewResourceRecord[]> = {
      当前采样: [
        ...
        { label: "磁盘使用率", value: `${snapshot.diskPercent}%`, delta: "仓库所在卷", values: spark(snapshot.diskPercent) },
    ...
    return OverviewSummaryPayloadSchema.parse({
      ...
      metrics: [
        ...
        { label: "磁盘使用率", value: String(snapshot.diskPercent), suffix: "%", delta: `${snapshot.diskFreeGb} GB 可用`, icon: "globe", tone: snapshot.diskPercent >= 90 ? "red" : snapshot.diskPercent >= 80 ? "orange" : "blue", line: spark(snapshot.diskPercent) },
  ...
  private buildRisks(snapshot: PlatformSnapshot): OverviewRiskRecord[] {
    ...
    addUsageRisk("risk-disk", "磁盘使用率偏高", snapshot.diskPercent, 90, 80, "仓库所在卷", "%");
