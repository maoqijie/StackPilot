...
export function createControllerServices(platform: PlatformAdapter, repoRoot: string, config: ControllerConfig, agentRepository?: AgentControlRepository): Services {
  ...
  return {
    ...
    files: new FileService(config.fileRoots),
...
export function createStackPilotApp(options: AppOptions = {}): RequestListener {
  ...
  return async (request: IncomingMessage, response: ServerResponse) => {
    ...
    try {
      ...
      const parsedBody = isWriteMethod(method) ? await readJsonRequest(request, bodyLimit, isFileUpload) : { value: {}, raw: Buffer.alloc(0) };
