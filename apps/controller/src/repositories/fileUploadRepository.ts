import type { ResumableFileUploadRecord } from "@stackpilot/contracts";
import type Database from "better-sqlite3";

export type StoredFileUpload = ResumableFileUploadRecord & { ownerUserId: string; temporaryPath: string; idempotencyKey: string };
type FileUploadRow = {
  ...
  status: ResumableFileUploadRecord["status"]; sha256: string | null; error_message: string | null; idempotency_key: string;
...
export class FileUploadRepository {
  ...
  updateProgress(id: string, expectedOffset: number, receivedBytes: number, status: ResumableFileUploadRecord["status"]): boolean {
  ...
  updateState(id: string, status: ResumableFileUploadRecord["status"], values: { sha256?: string | null; errorMessage?: string | null; completedAt?: string | null } = {}): void {
