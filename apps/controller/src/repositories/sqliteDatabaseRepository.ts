...
export class SqliteDatabaseRepository implements DatabaseRepository {
  ...
  private readonly resultKey: Buffer;
  constructor(private readonly database: Database.Database, masterKey: Buffer) {
    ...
    this.resultKey = deriveKey(masterKey, "database-operation-result-v1");
  }
  ...
  updateOperation(nodeId:string,update:AgentDatabaseOperationUpdate):DatabaseOperation{return this.database.transaction(()=>{const row=this.database.prepare("SELECT * FROM database_operations WHERE id=? AND node_id=?").get(update.operationId,nodeId)as Row|undefined;if(!row)throw new DatabaseRepositoryError(404,"NOT_FOUND","数据库操作不存在");const terminal=["succeeded","failed","cancelled"].includes(row.status as string);if(terminal&&row.status===update.status&&row.version===(update.version+1))return this.operation(row);if(row.version!==update.version)throw new DatabaseRepositoryError(409,"BAD_REQUEST","数据库操作版本不匹配");if(row.status===update.status)return this.operation(row);if(terminal)return this.operation(row);const allowed=row.status==="queued"?["running","succeeded","failed"]:row.status==="running"?["succeeded","failed"]:[];if(!allowed.includes(update.status))throw new DatabaseRepositoryError(409,"BAD_REQUEST","数据库操作状态转换无效");const credential=update.credentialEnvelope,result=update.result;if(credential&&row.kind!=="install")throw new DatabaseRepositoryError(400,"BAD_REQUEST","只有安装操作可以返回一次性凭据密文");if(credential&&(Date.parse(credential.expiresAt)>Date.parse(update.updatedAt)+5*60_000||Date.parse(credential.expiresAt)<=Date.parse(update.updatedAt)))throw new DatabaseRepositoryError(400,"BAD_REQUEST","一次性凭据密文有效期必须在五分钟内");if(result&&result.kind!==row.kind)throw new DatabaseRepositoryError(400,"BAD_REQUEST","数据库操作结果类型不匹配");const resultRequired=["backup","explain","install","restore"].includes(row.kind as string);if(update.status==="succeeded"&&resultRequired&&!result)throw new DatabaseRepositoryError(400,"BAD_REQUEST","数据库操作成功状态缺少结构化结果");if(update.status==="succeeded"&&row.kind==="install"&&!credential)throw new DatabaseRepositoryError(400,"BAD_REQUEST","安装成功状态缺少一次性凭据密文");const encryptedResult=result?encryptValue(this.resultKey,Buffer.from(JSON.stringify(result))):null,resultExpiresAt=result?.kind==="explain"?new Date(Date.parse(update.updatedAt)+7*86_400_000).toISOString():null;this.database.prepare("UPDATE database_operations SET status=?,version=version+1,error_code=?,error_message=?,credential_ciphertext=?,credential_expires_at=?,result_key_version=?,result_nonce=?,result_ciphertext=?,result_tag=?,result_expires_at=?,updated_at=?,completed_at=? WHERE id=? AND version=?").run(update.status,update.errorCode,update.errorMessage,credential?.ciphertext??null,credential?.expiresAt??null,encryptedResult?.keyVersion??null,encryptedResult?.nonce??null,encryptedResult?.ciphertext??null,encryptedResult?.tag??null,resultExpiresAt,update.updatedAt,update.status==="running"?null:update.updatedAt,update.operationId,update.version);this.reconcileOperationResult(row,update,result);return this.operation(this.database.prepare("SELECT * FROM database_operations WHERE id=?").get(update.operationId)as Row);})();}
  ...
  purgeExpiredSensitiveData(now:string){return this.database.transaction(()=>({
    sql:this.purgeExpiredSql(now),
    credentials:this.database.prepare("UPDATE database_operations SET credential_ciphertext=NULL,credential_expires_at=NULL WHERE credential_expires_at IS NOT NULL AND credential_expires_at<=?").run(now).changes,
    operationResults:this.database.prepare("UPDATE database_operations SET result_key_version=NULL,result_nonce=NULL,result_ciphertext=NULL,result_tag=NULL,result_expires_at=NULL WHERE result_expires_at IS NOT NULL AND result_expires_at<=?").run(now).changes,
  }))();}
  ...
  private operation(row:Row){const credentialCurrent=row.credential_ciphertext&&row.credential_expires_at&&Date.parse(row.credential_expires_at as string)>Date.now();const result=row.result_ciphertext&&row.result_nonce&&row.result_tag?JSON.parse(decryptValue(this.resultKey,{keyVersion:row.result_key_version as number,nonce:row.result_nonce as Buffer,ciphertext:row.result_ciphertext as Buffer,tag:row.result_tag as Buffer}).toString())as DatabaseOperationResult:null;return DatabaseOperationSchema.parse({id:row.id,kind:row.kind,nodeId:row.node_id,instanceId:row.instance_id,status:row.status,version:row.version,errorCode:row.error_code,errorMessage:row.error_message,requestedBy:row.requested_by,requestId:row.request_id,createdAt:row.created_at,updatedAt:row.updated_at,completedAt:row.completed_at,credentialEnvelope:credentialCurrent?{algorithm:"RSA-OAEP-256",ciphertext:row.credential_ciphertext,expiresAt:row.credential_expires_at}:null,result});}
  private reconcileOperationResult(row:Row,update:AgentDatabaseOperationUpdate,result:DatabaseOperationResult|null){
    if(row.kind!=="backup")return;const job=this.database.prepare("SELECT * FROM database_backup_jobs WHERE operation_id=?").get(row.id)as Row|undefined;if(!job)return;
    if(update.status==="running"){this.database.prepare("UPDATE database_backup_jobs SET status='running',started_at=COALESCE(started_at,?) WHERE id=?").run(update.updatedAt,job.id);return;}
    if(update.status==="failed"){this.database.prepare("UPDATE database_backup_jobs SET status='failed',completed_at=?,error_code=? WHERE id=?").run(update.updatedAt,update.errorCode,job.id);return;}
    if(update.status!=="succeeded"||result?.kind!=="backup")throw new DatabaseRepositoryError(400,"BAD_REQUEST","备份成功状态缺少 manifest 结果");
    this.database.prepare("UPDATE database_backup_jobs SET status='succeeded',started_at=COALESCE(started_at,?),completed_at=?,size_bytes=?,manifest_version=?,checksum=?,error_code=NULL WHERE id=?").run(result.createdAt,update.updatedAt,result.sizeBytes,result.manifestVersion,result.checksum,job.id);
    this.database.prepare(`INSERT OR IGNORE INTO database_restore_points(id,job_id,instance_id,created_at,size_bytes,checksum,database_version,manifest_version,verified_at,drill_status,drilled_at)
      VALUES(?,?,?,?,?,?,?,?,?,'not_started',NULL)`).run(result.restorePointId,job.id,row.instance_id,result.createdAt,result.sizeBytes,result.checksum,result.databaseVersion,result.manifestVersion,update.updatedAt);
  }
