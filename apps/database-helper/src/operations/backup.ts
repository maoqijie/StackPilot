...
export type BackupManifestFile = { name: string; sizeBytes: number; sha256: string; databaseName: string | null };
export type BackupManifest = { version: 1; id: string; instanceId: string; engine: string; databaseVersion: string | null; createdAt: string; files: BackupManifestFile[] };
export const hashFile = (path: string) => new Promise<string>((resolve, reject) => { const hash = createHash("sha256"); createReadStream(path).on("error", reject).on("data", (chunk) => hash.update(chunk)).on("end", () => resolve(hash.digest("hex"))); });
...
export class DatabaseBackupService {
  ...
  async create(instance: ManagedInstance, credential: InstanceCredential, retentionCount: number) {
    const id = randomUUID(), directory = join(instance.backupDirectory, id); await mkdir(instance.backupDirectory, { recursive: true, mode: 0o700 }); await chmod(instance.backupDirectory, 0o700); const release = await acquireInstanceLock(instance.backupDirectory);
    try {
      await this.capacityPreflight(instance, credential); await mkdir(directory, { recursive: false, mode: 0o700 });
      ...
      const entries = await Promise.all(files.map(async (file) => { const path = join(directory, file.name), metadata = await stat(path); return { ...file, sizeBytes: metadata.size, sha256: await hashFile(path) }; }));
      const manifest: BackupManifest = { version: 1, id, instanceId: instance.id, engine: instance.engine, databaseVersion: instance.version, createdAt: new Date().toISOString(), files: entries };
  ...
  private async postgres(instance: ManagedInstance, credential: InstanceCredential, directory: string) {
    ...
    await this.runner.run("pg_dumpall", [...common, "--globals-only", "--no-tablespaces"], { outputPath: join(directory, "globals.sql"), credentialEnvironment: env });
    ...
    const files: Array<{ name: string; databaseName: string | null }> = [{ name: "globals.sql", databaseName: null }];
    for (const name of names) {
      ...
      await this.runner.run("pg_dump", [...common, "--format=custom", "--no-tablespaces", name], { outputPath: join(directory, file), credentialEnvironment: env, timeoutMs: 30 * 60_000 }); files.push({ name: file, databaseName: name });
    }
    return files;
  }
  private async mysql(instance: ManagedInstance, credential: InstanceCredential, directory: string) {
    ...
    return [{ name: "all-databases.sql", databaseName: null }];
  }
