...
const upgraded=new Database(databasePath,{readonly:true});if(upgraded.prepare("SELECT max(version) AS version FROM schema_migrations").get().version!==4||upgraded.prepare("SELECT username FROM users WHERE username=?").get(marker)?.username!==marker)throw new Error("升级后数据验证失败");upgraded.close();
