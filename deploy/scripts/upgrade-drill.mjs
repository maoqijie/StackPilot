...
const upgraded=new Database(databasePath,{readonly:true});if(upgraded.prepare("SELECT max(version) AS version FROM schema_migrations").get().version!==6||upgraded.prepare("SELECT username FROM users WHERE username=?").get(marker)?.username!==marker)throw new Error("升级后数据验证失败");upgraded.close();
...
process.stdout.write(`schema 1→6 升级、SHA-256 校验、测试实例删除及 schema 1 回滚恢复演练通过：${work}\n`);
