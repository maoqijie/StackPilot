...
const disk=statfsSync(existsSync(databasePath)?databasePath:resolve(databasePath,".."));checks.push([disk.bavail*disk.bsize>=requiredFree,"数据库卷至少有 512 MiB 可用空间"]);
if(existsSync(databasePath)){const db=new Database(databasePath,{readonly:true});const integrity=db.pragma("integrity_check",{simple:true});const schema=(db.prepare("SELECT max(version) AS version FROM schema_migrations").get()).version;checks.push([integrity==="ok","SQLite integrity_check 通过"]);checks.push([[1,2,3,4,5,6].includes(schema),`数据库 schema ${schema} 可升级到 6`]);const versions=db.prepare("SELECT DISTINCT json_extract(payload,'$.agentVersion') AS version FROM agent_nodes WHERE revoked_at IS NULL").all().map(row=>row.version).filter(Boolean);checks.push([versions.every(version=>String(version).startsWith("0.1.")||String(version).startsWith("0.2.")||String(version).startsWith("0.3.")),"已注册 Agent 版本在 0.1.x/0.2.x/0.3.x 兼容范围"]);db.close();}
