...
StackPilot `0.2.0-preview.2` is a preview release, not a stable production release. Support below means the release gates are designed for that target; it does not imply an SLA.
...
| Controller | `0.2.0-preview.2` | must match Web release version |
...
| --- | --- | --- | --- | --- |
| `0.2.x` | `0.1.x` or `0.2.x` | `1.x` | 2 | supported preview combination |
| `0.2.x` | any | non-`1.x` | any | rejected before task processing |
| `0.2.x` | any | `1.x` | 1 | upgrade required; back up before migration |
| `0.2.x` | any | `1.x` | greater than 2 | refuse startup/upgrade; newer database cannot be opened safely |

Schema 2 has no automatic down migration. Rollback to a schema-1 Controller means restoring the verified pre-upgrade schema-1 backup.
