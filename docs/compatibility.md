...
StackPilot `0.3.0-preview.14` is a preview release, not a stable production release. Support below means the release gates are designed for that target; it does not imply an SLA.
...
| Agent/database-helper OS | Not yet certified for `0.3.0-preview.14` | drivers target Debian 12, Ubuntu 24.04, Rocky/Alma 9, Fedora 42, Alpine 3.22 and Arch x86_64, but support remains blocked until pinned per-OS install, multi-instance, backup and restore CI gates pass |
...
| Controller | `0.3.0-preview.14` | must match Web release version |
