...
| Database | embedded SQLite, schema `6` | direct upgrade from schemas 1, 2, 3, 4 and 5; restore accepts schemas 1, 2, 3, 4, 5 and 6 |
...
| --- | --- | --- | --- | --- |
| `0.2.x` | `0.1.x` or `0.2.x` | `1.x` | 6 | supported preview combination |
| `0.2.x` | any | non-`1.x` | any | rejected before task processing |
| `0.2.x` | any | `1.x` | 1, 2, 3, 4 or 5 | upgrade required; back up before migration |
| `0.2.x` | any | `1.x` | greater than 6 | refuse startup/upgrade; newer database cannot be opened safely |

Schema 6 has no automatic down migration. Rollback to a Controller that only supports an older schema means restoring its verified pre-upgrade backup.
