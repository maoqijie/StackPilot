...
StackPilot `0.3.0-preview.27` is a preview release, not a stable production release. Support below means the release gates are designed for that target; it does not imply an SLA.
...
| Agent/database-helper OS | Not yet certified for `0.3.0-preview.27` | drivers target Debian 12, Ubuntu 24.04 LTS, Rocky/Alma 9, Fedora 42, Alpine 3.22 and Arch x86_64, but support remains blocked until pinned per-OS install, multi-instance, backup and restore CI gates pass |
...
| Controller state | embedded SQLite, schema `8` | direct upgrade from schemas 1 through 7; rollback requires the verified pre-upgrade backup |
...
| Controller | `0.3.0-preview.27` | must match Web release version |
...
| --- | --- | --- | --- | --- |
| `0.3.x` | `0.1.x` or `0.2.x` | `1.0` | 8 | accepted without database capabilities |
| `0.3.x` | `0.3.x` | `1.1` | 8 | supported database-capable preview combination |
| `0.3.x` | any | any version other than `1.0` or `1.1` | any | rejected before task processing |
| `0.3.x` | any | `1.0` or `1.1` | 1 through 7 | upgrade required; back up before migration |
| `0.3.x` | any | `1.0` or `1.1` | greater than 8 | refuse startup/upgrade; newer database cannot be opened safely |

Schema 8 has no automatic down migration. Rollback to a Controller that only supports an older schema means restoring its verified pre-upgrade backup. Roll out Controller/Web first, then site/database helpers, then Agent; a protocol `1.1` Agent must not be connected to an older Controller.
