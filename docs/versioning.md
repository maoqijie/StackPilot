...
Controller and Web versions must match. Controller-Agent compatibility is independently negotiated through protocol `1.0`; `0.1.x` and `0.2.x` Agents are accepted only while they use protocol major 1 and server policy permits their declared task capability. Database schema 4 can be upgraded directly from schema 1, 2, or 3. There is no automatic down migration.
