...
For native certificate renewal, install the socket-activated helper instead of granting Agent sudo access. Keep `/run/stackpilot-cert-helper/helper.sock` at `0660 root:stackpilot-cert`, review the helper unit before upgrades, and authorize `sites.certificates.renew` only for intended nodes. Docker Agents intentionally cannot renew host certificates. Renewal tasks are never retried automatically after an unknown outcome.
