...
For production restore, stop Controller first. `db:restore` accepts supported schema 1, 2, 3, 4 or 5 backups, validates SQLite integrity, installs atomically and retains the displaced database as `.before-restore`. A restored database must be paired with a compatible application version or migrated again after investigation.
