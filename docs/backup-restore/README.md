...
For production restore, stop Controller first. `db:restore` accepts supported schema 1 or 2 backups, validates SQLite integrity, installs atomically and retains the displaced database as `.before-restore`. A restored schema-1 database must be paired with the schema-1 application for rollback or migrated again after investigation.
