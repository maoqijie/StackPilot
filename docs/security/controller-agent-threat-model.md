...
Linux certificate renewal crosses a minimal root boundary through a systemd-activated Unix socket. The socket is `0660 root:stackpilot-cert` and is never exposed on TCP. The one-shot helper accepts only `status`, or `renew` plus an opaque certificate ID. It remaps that ID from active Nginx public certificate configuration and calls only fixed absolute Certbot, Nginx, and systemctl commands through `execFile`. It accepts no caller path, executable, arbitrary arguments, process environment override, or shell text. Certbot `live/` public-certificate links must resolve to same-name public files under `archive/`; private-key links and mixed certificate/private-key PEM files are rejected. Renewal tasks execute at most once. A task can be cancelled only before dispatch; interrupted unknown outcomes retain the certificate lock and are not replayed automatically.
