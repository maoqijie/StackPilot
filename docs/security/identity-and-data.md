...
Production retention and isolated restore drills are documented in [backup and restore](../backup-restore/README.md). Schema 6 cannot be down-migrated; rollback to an older schema restores the verified pre-upgrade backup with the matching prior application version.
